...
def get_data(filters):
	...
	# 	from `tabAccount` where company=%s and parent_account=null order by lft""",
	# 	filters.company,
	# 	as_dict=True,
	# )
	accounts = frappe.db.sql(
	...
	# keep only:
	# - top-level accounts (no parent or indent 0)
	# - the separator row ({})
	# - the Total row
	top_level_data = []
	for row in data:
		# empty separator row
		if not row:
			top_level_data.append(row)
			continue

		is_total_row = row.get("account") == "'" + _("Total") + "'"
		is_top_parent = (not row.get("parent_account")) or row.get("indent") == 0

		if is_top_parent or is_total_row:
			top_level_data.append(row)
